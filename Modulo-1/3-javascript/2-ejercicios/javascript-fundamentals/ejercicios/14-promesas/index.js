...
const getPasta = (pasta) => resolvePasta(pasta).then((message) => console.log(message));;
...
const addAnimals = () => {
  return getAnimals().then((animals) => new Promise((resolve) => {
    animals.push('tiger');
    resolve(animals);
  })
...
const extraPromise = (isResolved) => {
  var promise = new Promise((resolve, reject) => {
    if (isResolved) {
      resolve('estoy resuelto');
    } else {
      reject('he sido rechazado');
    }
  });
  return promise.then((result) => console.log(result)).catch((error) => console.log(error));
};

extraPromise(true);
/*
extraPromise(false).then((result) => console.log(result)).catch((error) => console.log(error));
*/
